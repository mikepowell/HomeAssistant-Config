...
class RyobiGDO:
    ...
    def get_api_key(self):
        ...
        for attempt in range(5):
            try:
                resp = requests.post('https://{}/{}'.format(self.HOST_URI, self.LOGIN_ENDPOINT), timeout=self.REQUEST_TIMEOUT, params={
    ...
    def check_device_id(self):
        ...
        for attempt in range(5):
            try:
                resp = requests.get("https://{}/{}".format(self.HOST_URI, self.DEVICE_GET_ENDPOINT), timeout=self.REQUEST_TIMEOUT, params={
                    'username': self.username,
                    'password': self.password
                })
            except requests.exceptions.RequestException:
    ...
    def update(self):
        ...
        for attempt in range(5):
            try:
                resp = requests.get("https://{}/{}/{}".format(self.HOST_URI, self.DEVICE_GET_ENDPOINT, self.device_id), timeout=self.REQUEST_TIMEOUT, params={
                    'username': self.username,
                    'password': self.password
                })
            except requests.exceptions.RequestException:
